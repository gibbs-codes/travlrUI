...
export default function Recommendations() {
  ...
  const handleRerunFlights = async () => {
    try {
      await flightService.rerunRecommendations(tripId);
      await fetchTrip(); // Refresh the data
    } catch (error) {
      console.error('Failed to rerun flights:', error);
    }
  };

  const handleRerunHotels = async () => {
    try {
      await hotelService.rerunRecommendations(tripId);
      await fetchTrip(); // Refresh the data
    } catch (error) {
      console.error('Failed to rerun hotels:', error);
    }
  };

  const handleRerunExperiences = async () => {
    try {
      await experienceService.rerunRecommendations(tripId);
      await fetchTrip(); // Refresh the data
    } catch (error) {
      console.error('Failed to rerun experiences:', error);
    }
  };

  const handleRerunRestaurants = async () => {
    try {
      await restaurantService.rerunRecommendations(tripId);
      await fetchTrip(); // Refresh the data
    } catch (error) {
      console.error('Failed to rerun restaurants:', error);
    }
  };
                      ...
                      {!isLoading &&
                        data?.transit.map((option) => (
                          <ExperienceCard
                            ...
                            onRerun={handleRerunExperiences}
                          />
                        ))}
                      ...
                      {!isLoading &&
                        data?.restaurants.map((restaurant) => (
                          <RestaurantCard
                            ...
                            onRerun={handleRerunRestaurants}
                          />
                        ))}
